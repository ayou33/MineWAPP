...
export default function FormItem (props: FormItemProps) {
  // Reserve the error slot whenever the error prop is wired up (even if null).
  const hasSlot = () => props.error !== undefined || props.helper !== undefined
  ...
  return (
    <div class="flex flex-col gap-1">
      ...
      {/*
        Fixed-height slot: always occupies 1.125rem when error is wired up.
        Height NEVER changes → zero layout shift → no page-level flicker.
        Visibility is controlled by opacity alone (CSS transition, no DOM diff).
      */}
      <Show when={hasSlot()}>
        <div class="h-[1.125rem] flex items-center overflow-hidden">
          ...
          >
            {/* Icon always reserves its width; visibility prevents ghost icon flicker */}
            <Icon
              name="AlertCircle"
              size={12}
